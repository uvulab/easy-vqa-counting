...
- An additional option is to use element-wise multiplication along with Gated Tanh Units [(GTU)](https://arxiv.org/abs/1707.07998). These GTUs are meant to learn non-linear transformations within the network. GTUs have shown a strong empirical advantage over traditional ReLU or tanh layers alone. When dealing with small datasets (e.g. 300 images for training), GTUs have shown to speed up the convergence and performance.
...
Example 2:
```python
python train.py data/box_test_v3 count
```
- This line uses the dataset located in <em>data/box_test_v3</em>, using the <em>count</em> model, since there is no fusion method explicitly indicated it will use the Gated Tanh Units, and the feature maps in the fusion will have the default size <em>32</em>.
